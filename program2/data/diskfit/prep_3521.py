# Build DiskFit .inp files for NGC 3521 from the THINGS MOM1 map + dB08 Table 2 geometry.
import numpy as np
...
MOM1 = "../../../data/things_pilot/NGC_3521_RO_MOM1_THINGS.FITS"
RA, DEC = "11:05:48.6", "-00:02:09.2"     # dB08 Table 2
VSYS, INC, PA = 803.5, 72.7, 339.8        # dB08 Table 2
PIXSCALE = 1.5                             # THINGS ''/pix
RING_STEP_PIX, RING_MAX_PIX = 10, 230      # rings every 15'' out to ~345''
...
def inp(name, boot):
    return f"""NGC3521 THINGS MOM1, dB08 geometry # pixscale {PIXSCALE}''/pix
...
import os
os.makedirs("OUT_3521", exist_ok=True)
open("ngc3521_disk.inp", "w").write(inp("disk3521", False))
open("ngc3521_boot.inp", "w").write(inp("boot3521", True))
print(f"center pix: ({x:.1f},{y:.1f})  eps={eps:.3f}  rings: {RING_STEP_PIX}-{RING_MAX_PIX} pix")
print("wrote ngc3521_disk.inp (fast, no errors) and ngc3521_boot.inp (bootstrap)")
