...
def bounds(N,rmax):
    ...
    lo+=[0.05]; hi+=[0.50]    # alpha bounds (physical Einasto shape range)
...
def main():
    ...
    D=pd.DataFrame(res); D.to_csv("../data/einasto_free_alpha.csv",index=False)
    n=len(D); 
    print(f"free-alpha Einasto: {n} galaxies, {time.time()-t0:.0f}s\n")
